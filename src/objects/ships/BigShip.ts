...
export class BigShip extends Battler {
    fireInterval: number = 60 * 6;
    evasionInterval: number = 60 * 6;
    thrust = 0;
    turnSpeed = .001;
    maxFuel = 60 * 5;
    maxHealth = 25;
    ...
    updateGraphics() {
        ...
        for (let i = 0; i <= 2 * Math.PI; i += Math.PI / 4) {
            myShape.drawCircle(this.size * Math.cos(i), this.size * Math.sin(i), this.size / 8)
        }
        myShape.endFill();
